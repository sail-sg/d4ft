...
class molecule(object):
  ...
  def _init_param(self, seed=123):
    ...
    return random.normal(
      key, [self.basis_num, self.basis_num, self.basis_num]
    ) / self.basis_num**0.5
  ...
  def train(
    ...
    **args
    ...
    if sample_method == 'pyscf':
      ...
      mol = gto.Mole(basis='sto-3g', spin=0)
    ...
    if sample_method == 'pyscf':
      ...
      def update(params, opt_state, key):
        loss = lambda params: E_gs(
          wave_fun_N, self.pyscf_grid, params, self.tot_electron, self.nuclei,
          weight
        )
      ...
      def update(params, opt_state, key):
        ...
        loss = lambda params: E_gs(
          wave_fun_N, meshgrid, params, self.tot_electron, self.nuclei, weight
        )
        loss_value, grads = jax.value_and_grad(loss)(params)
        updates, opt_state = optimizer.update(grads, opt_state)
        params = optax.apply_updates(params, updates)
    ...
    for i in range(epoch):
      ...
      if (i + 1) % 10 == 0:
        ...
        if if_val:
          E_gs_val.append(
            E_gs(
              wave_fun_N, meshgrid, params, self.tot_electron, self.nuclei,
              weight
            )
          )
    ...
    print(
      'E_kinetic ',
      E_kinetic(wave_fun_N, meshgrid, params, self.tot_electron, weight)
    )
    print(
      'E_ext: ',
      E_ext(
        wave_fun_N, meshgrid, self.nuclei, params, self.tot_electron, weight
      )
    )
    print(
      'E_Hartree: ',
      E_Hartree(wave_fun_N, meshgrid, params, self.tot_electron, weight)
    )
    print(
      'E_xc: ',
      E_XC_LDA(wave_fun_N, meshgrid, params, self.tot_electron, weight)
    )
