...
def batch_sampler(grids, weights, batchsize, seed=1):
    ...
    output_grid = []
    output_weight = []
    batch_idx = [idx[sample_boundary[i]:sample_boundary[i+1]] for i in jnp.arange(nbatch)]
...
def simple_grid(key, limit, cellsize, n=100, verbose=False):
    ...
    centers = cellsize*jnp.array(jnp.meshgrid(*[jnp.arange(cell_num)-int(cell_num/2) for i in np.arange(3)])).reshape(3, -1).transpose()
...
def poisson_disc_sampling_3d(limit=1, radius=0.1, k=30, n=100):
    ...
    grid_index_array = -jnp.ones([grid_num_per_axis,
                                  grid_num_per_axis,
                                  grid_num_per_axis], dtype=jnp.int8)
    ...
    def get_neighborhood(sample_coord):
        ...
        grid_index = jnp.array([((sample_coord[0]+limit)/grid_r).astype(int),
                    ((sample_coord[1]+limit)/grid_r).astype(int),
                    ((sample_coord[2]+limit)/grid_r).astype(int)])
        neigh_index = get_index(grid_index)
        index_in_sample_array = vmap(lambda idx: jnp.array(grid_index_array[idx[0], idx[1], idx[2]]))(neigh_index)
...
#         output = sample_array[index_in_sample_array]
    ...
    def get_index(grid_index):
        ...
        index_ = np.array(np.meshgrid(*[np.arange(5)-2 for i in range(3)])).reshape(3, -1).transpose()
...
#             return False
    ...
    grid_index_array = grid_index_array.at[int((activate_array[-1][0]+limit)/grid_r),
    ...
    while len(activate_array)>0:
        ...
        if in_nei is not False:
            ...
            grid_index_array = grid_index_array.at[int((in_nei[0]+limit)/grid_r),
