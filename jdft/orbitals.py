...
class Pople(Basis):
  ...
  def __call__(self, r):
    ...
    const = const1 * (((8 * self.exponent_mat) ** const2 * const3) /
            const4) ** 0.5
    output = const * jnp.prod(jnp.power(r-self.coord, self.ijk), axis=1, keepdims=True)
    def fc(c): return jnp.linalg.norm(r - c) ** 2

    output *= self.coeff_mat * jnp.exp(-self.exponent_mat * jnp.expand_dims(jax.vmap(fc)(self.coord), 1))
