...
def sgd_solver(
  ...
  hamiltonian = calc_hamiltonian(mol, dft_cfg)
  e_fns = get_energy_fns(hamiltonian)
  ...
  def update(params, opt_state):
    ...
    mo_coeff = hamiltonian.mo_coeff_fn(params)  # (2*nao, nao)
    ...
    grad = sum(grads)
    e_total = e_kin + e_ext + e_eri + e_xc + hamiltonian.e_nuc
    updates, opt_state = optimizer.update(grad, opt_state)
    params = optax.apply_updates(params, updates)
    energies = Energies(e_total, e_kin, e_ext, e_eri, e_xc, hamiltonian.e_nuc)
    grads = Grads(*e_grads)
  ...
  for step in range(optim_cfg.epochs):
    ...
    mo_coeff = hamiltonian.mo_coeff_fn(params)
