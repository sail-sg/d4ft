...
def sgd(
    cfg: D4FTConfig, H: Hamiltonian, params: hk.Params, key: jax.Array
    ...
    def meta_step(state: TrainingState, meta_state: TrainingState):
        ...
        return (
            TrainingState(new_meta_params, meta_opt_state, meta_state.rng_key),
            new_state,
            energies,
            mo_grads,
        )
    ...
    for step in range(solver_cfg.epochs):
        ...
        if (
            ...
            and "center" in new_state.params["~"]
            and step % 2 == 0
        ):
        ...
        if (
            wandb.run is not None
            and "center_flob" in new_state.params["~"]
            and step % 2 == 0
        ):
...
def plot_centers_3d(unique_coords, pgto, coeffs, step, atom_indices, params=None):
    ...
    hover_text = []
    for i, (l, pos, exp, a_idx) in enumerate(
        zip(l_values, basis, exponents, atom_indices)
    ):
        text = [
            f"PGTO<br>",
    ...
    # Prepare statistics
    stats = {
        "max_distance_to_atom": float(
            np.linalg.norm(basis[:, None] - atoms[None], axis=2).min(axis=1).max()
        ),
        "max_angular": int(l_values.max()),
        "mean_exponent": float(exponents.mean()),
        "angular_distribution": wandb.Histogram(l_values),
        "exponent_distribution": wandb.Histogram(log_exponents),
    }
    ...
    # Log to wandb
    wandb.log(
        {"centers_plot": wandb.Html(fig.to_html()), "basis_stats": stats, "step": step}
    )
