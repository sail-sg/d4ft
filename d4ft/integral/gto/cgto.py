...
def reparameterize(
    ...
    for i, element_basis in enumerate(cgto.basis):
        ...
        for cgto_i in element_basis:
            ...
            assert not isinstance(cgto_i[1], float), (
            ...
            for cid in range(1, 1 + n_coeffs):  # 0-idx is exponent
                ...
                for angular in get_cart_angular_vec(total_angular):
                        ...
                        if optim_exp:
                            exponent = jax.nn.softplus(
                                hk.get_parameter(
                                    ...
                                    shape=(),
                                    init=make_constant_fn(
                                        inv_softplus(jnp.array(exponent))
                                    ),
                                )
                            )
                        coeff = pgto_i[cid]
                    ...
                    N = pgto_i.norm_inv()
                    normalized_coeffs_i = (
                        normalize_cgto_coeff(pgto_i, jnp.array(coeffs_i) * N) / N
                    )
...
def build_cgto_from_mol(mol: Mol) -> CGTO:
    ...
    logging.info(f"there are {sum(cgto_splits)} (non-unique) PGTOs in spherical form")
...
class PGTO(NamedTuple):
  ...
  trainable."""
    ...
    def concat(pgtos: Sequence[PGTO]) -> PGTO:
        ...
        angular, center, exponent = zip(*pgtos)
        return PGTO(
            np.concatenate(angular), jnp.concatenate(center), jnp.concatenate(exponent)
        )
    ...
    def to_hk(
        ...
        if "center_flob" in optimizable_params:  # floating orbitals
            ...
            center_params = hk.get_parameter(
                "center_flob", center_init.shape, init=make_constant_fn(center_init)
            )

            center = jnp.repeat(
                center_params,
                jnp.array(self.atom_splits),  # [1,2]
                axis=0,
    ...
    def get_mo_coeff(
        ...
        if use_hk:
            mo_params = hk.get_parameter(
                "mo_params",
                shape,
                init=hk.initializers.RandomNormal(stddev=1.0 / math.sqrt(nmo)),
            )
        else:
