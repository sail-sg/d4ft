...
PLAS_HOST_DEVICE void vertical_0_0_c_0(int i, FLOAT* pq, FLOAT* qc,
  ...
  for (int j = 0; j < max_cd[i]; ++j) {
    ...
    for (int k = 0; k <= Ms[i]; ++k) {
      FLOAT I_mp1_k = wq_i * I_0_cm1[k] + (-cm1/2/eta*rho/eta) * I_0_cm2[k];
...
template <typename FLOAT>
PLAS_HOST_DEVICE void vertical_a_0_c_0(int i, FLOAT I[][MAX_XYZ+1],
                      FLOAT* ab, FLOAT* cd, FLOAT* pa, FLOAT*pq,
                      FLOAT rho, FLOAT zeta, FLOAT eta,
                      int* na, int* nb, int* nc, int* nd,
                      int* Ms, int* min_a, int* min_c, int* max_ab, int* max_cd,
                      FLOAT* out) {
  ...
  for (int j = 0; j <= max_ab[i]; ++j) {
    ...
    for (int k = 0; k <= max_cd[i]; ++k) {
      for (int l = 0; l <= Ms[i]; ++l) {
        FLOAT I_mp1_kl = wp_i * I_am1[k][l] + (-am1/2/zeta*rho/zeta) * I_am2[k][l];
        // inplace write
...
template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT zeta(FLOAT za, FLOAT zb) {
  return za + zb;
}

template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT xi(FLOAT za, FLOAT zb) {
  return za * zb / zeta(za, zb);
}

template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT rp(FLOAT ra, FLOAT rb, FLOAT za, FLOAT zb) {
  return (za * ra + zb * rb) / (za + zb);
}

template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT rho(FLOAT zeta, FLOAT eta) {
  return zeta * eta / (zeta + eta);
}

template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT T(FLOAT rho, FLOAT* pq) {
  return rho * (pq[0]*pq[0] + pq[1]*pq[1] + pq[2]*pq[2]);
}

// translate above python code into C code
template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT K(FLOAT z1, FLOAT z2, FLOAT* r1, FLOAT* r2) {
  FLOAT d_squared = 0;
  for (int i = 0; i < 3; ++i) {
...
template <typename FLOAT>
PLAS_HOST_DEVICE FLOAT eri(int nax, int nay, int naz,
  ...
  vertical_a_0_c_0(0, I, ab_, cd_, pa_, pq_, rho_, zeta_, eta_,
                   na, nb, nc, nd,
                   Ms, min_a, min_c, max_ab, max_cd,
                   out);
  // reset I to zero
  ...
  vertical_a_0_c_0(1, I, ab_, cd_, pa_, pq_, rho_, zeta_, eta_,
                   na, nb, nc, nd,
                   Ms, min_a, min_c, max_ab, max_cd,
                   out);
  // reset I to zero
  ...
  vertical_a_0_c_0(2, I, ab_, cd_, pa_, pq_, rho_, zeta_, eta_,
                   na, nb, nc, nd,
                   Ms, min_a, min_c, max_ab, max_cd,
                   out);
