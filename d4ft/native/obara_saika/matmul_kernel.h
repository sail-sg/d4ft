...
void matmul_baseline(input_it A, input_it B, int m, int n, int k, output_it C,
                     context_t& context) {
...
template <typename input_it, typename output_it>
void matmul_coalesce(input_it A, input_it B, int m, int n, int k, output_it C,
                     context_t& context) {
  typedef typename std::iterator_traits<input_it>::value_type type_t;
...
template <typename input_it, typename output_it>
void matmul_shmem(input_it A, input_it B, int m, int n, int k, output_it C,
                  context_t& context) {
  ...
  auto K = [=] PLAS_DEVICE(int tid, int cta) {
    ...
    for (int k_idx = 0; k_idx < k; k_idx += kBlockSize) {
      ...
      As[thread_row_idx * kBlockSize + thread_col_idx] =
          ldg(A + A_global_offset + thread_row_idx * k + thread_col_idx);
      Bs[thread_row_idx * kBlockSize + thread_col_idx] =
          ldg(B + B_global_offset + thread_row_idx * n + thread_col_idx);
      ...
      for (int acc_idx = 0; acc_idx < kBlockSize; ++acc_idx) {
        tmp += As[thread_row_idx * kBlockSize + acc_idx] *
               Bs[acc_idx * kBlockSize + thread_col_idx];
      }
...
template <typename input_it, typename output_it>
void matmul_1dblocktiling(input_it A, input_it B, int m, int n, int k,
                          output_it C, context_t& context) {
  ...
  auto K = [=] PLAS_DEVICE(int tid, int cta) {
    ...
    int C_global_offset =
        A_block_idx * n * kBlockSize + B_block_idx * kBlockSize;
    ...
    for (int k_idx = 0; k_idx < k; k_idx += kBK) {
      ...
      As[A_thread_row_idx * kBK + A_thread_col_idx] =
          ldg(A + A_global_offset + A_thread_row_idx * k + A_thread_col_idx);
      Bs[B_thread_row_idx * kBlockSize + B_thread_col_idx] =
          ldg(B + B_global_offset + B_thread_row_idx * n + B_thread_col_idx);
      ...
      for (int res_idx = 0; res_idx < kBK; ++res_idx) {
        for (int acc_idx = 0; acc_idx < kBK; ++acc_idx) {
          tmp[res_idx] +=
              As[(global_thread_row_idx * kBK + res_idx) * kBK + acc_idx] *
              Bs[acc_idx * kBlockSize + global_thread_col_idx];
        }
      }
    ...
    for (int res_idx = 0; res_idx < kBK; ++res_idx) {
      int cc = C_global_offset + (global_thread_row_idx * kBK + res_idx) * n +
               global_thread_col_idx;
      C[cc] = tmp[res_idx];
    }
  };
  cta_launch<launch_t>(K, num_ctas, context);
...
template <typename input_it, typename output_it>
void matmul_2dblocktiling(input_it A, input_it B, int m, int n, int k,
                          output_it C, context_t& context) {
  const int kBlockSize = 64;
  const int kBK = 8;
  ...
  auto K = [=] PLAS_DEVICE(int tid, int cta) {
    ...
    for (int k_idx = 0; k_idx < k; k_idx += kBK) {
      // load 64*8 from A to As, 8*64 from B to Bs
      for (size_t load_offset = 0; load_offset < kBlockSize;
           load_offset += A_stride) {
        As[(A_thread_row_idx + load_offset) * kBK + A_thread_col_idx] =
            ldg(A + A_global_offset + (A_thread_row_idx + load_offset) * k +
                A_thread_col_idx);
      }

      for (size_t load_offset = 0; load_offset < kBK; load_offset += B_stride) {
        Bs[(B_thread_row_idx + load_offset) * kBlockSize + B_thread_col_idx] =
            ldg(B + B_global_offset + (B_thread_row_idx + load_offset) * n +
                B_thread_col_idx);
      }
      ...
      for (int acc_idx = 0; acc_idx < kBK; ++acc_idx) {
        ...
        for (int res_idx = 0; res_idx < kBK; ++res_idx) {
          reg_A[res_idx] =
              As[(global_thread_row_idx * kBK + res_idx) * kBK + acc_idx];
          reg_B[res_idx] =
              Bs[acc_idx * kBlockSize + global_thread_col_idx * kBK + res_idx];
        }
        for (int res_idx_A = 0; res_idx_A < kBK; ++res_idx_A) {
          for (int res_idx_B = 0; res_idx_B < kBK; ++res_idx_B) {
            tmp[res_idx_A * kBK + res_idx_B] +=
                reg_A[res_idx_A] * reg_B[res_idx_B];
          }
        }
      }
      __syncthreads();
    ...
    for (int res_idx_A = 0; res_idx_A < kBK; ++res_idx_A) {
      for (int res_idx_B = 0; res_idx_B < kBK; ++res_idx_B) {
        int cc = C_global_offset +
                 (global_thread_row_idx * kBK + res_idx_A) * n +
                 global_thread_col_idx * kBK + res_idx_B;
...
template <typename input_it, typename output_it>
void matmul_2dblocktiling_vec(input_it A, input_it B, int m, int n, int k,
                              output_it C, context_t& context) {
  ...
  auto K = [=] PLAS_DEVICE(int tid, int cta) {
    ...
    for (int k_idx = 0; k_idx < k; k_idx += kBK) {
      ...
      float4 tmp_A = reinterpret_cast<float4*>(
          &A[A_global_offset + A_thread_row_idx * k + A_thread_col_idx * 4])[0];
      ...
      /*reinterpret_cast<float4*>
        (&Bs[B_thread_row_idx * kBlockSize + B_thread_col_idx * 4])[0] =
          reinterpret_cast<float4*>(&B[B_global_offset + B_thread_row_idx * n +
        B_thread_col_idx * 4])[0];*/
      float4 tmp_B = reinterpret_cast<float4*>(
          &B[B_global_offset + B_thread_row_idx * n + B_thread_col_idx * 4])[0];
      Bs[((B_thread_col_idx % 2) * 4 + B_thread_row_idx * 8 + 0) * 16 +
         B_thread_col_idx / 2] = tmp_B.x;
      Bs[((B_thread_col_idx % 2) * 4 + B_thread_row_idx * 8 + 1) * 16 +
         B_thread_col_idx / 2] = tmp_B.y;
      Bs[((B_thread_col_idx % 2) * 4 + B_thread_row_idx * 8 + 2) * 16 +
         B_thread_col_idx / 2] = tmp_B.z;
      Bs[((B_thread_col_idx % 2) * 4 + B_thread_row_idx * 8 + 3) * 16 +
         B_thread_col_idx / 2] = tmp_B.w;
      __syncthreads();
      ...
      for (int acc_idx = 0; acc_idx < kBK; ++acc_idx) {
        ...
        for (int res_idx = 0; res_idx < kBK; ++res_idx) {
          reg_A[res_idx] =
              As[acc_idx * kBlockSize + global_thread_row_idx * kBK + res_idx];
          // reg_B[res_idx] = Bs[acc_idx * kBlockSize + global_thread_col_idx *
          // kBK + res_idx];
          reg_B[res_idx] =
              Bs[(acc_idx * 8 + res_idx) * 16 + global_thread_col_idx];
        }
        for (int res_idx_A = 0; res_idx_A < kBK; ++res_idx_A) {
          for (int res_idx_B = 0; res_idx_B < kBK; ++res_idx_B) {
            tmp_res[res_idx_A * kBK + res_idx_B] +=
                reg_A[res_idx_A] * reg_B[res_idx_B];
          }
        }
      }
    ...
    for (int res_idx_A = 0; res_idx_A < kBK; ++res_idx_A) {
      for (int res_idx_B = 0; res_idx_B < kBK; res_idx_B += 4) {
        float4 tmp_C = reinterpret_cast<float4*>(
            &C[C_global_offset + (global_thread_row_idx * kBK + res_idx_A) * n +
