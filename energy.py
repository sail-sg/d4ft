...
def E_kinetic(wave_fun, meshgrid, params, N, limit=0.5):
    ...
    return -jnp.sum(batched_lap)/meshgrid.shape[0]*limit*2


def E_ext(wave_fun, meshgrid, nuclei, params, N, eps=1e-10, limit=0.5):
    '''
    input: 
        wave_fun: (3)-->(N)
        meshgrid: (D, 3)
    ...
    for k in range(num_ncl):
        ...
        output += jnp.sum(wave_at_grid**2 * jnp.expand_dims(x, 1))
    
    return output/meshgrid.shape[0]*limit*2


def E_XC_LDA(wave_fun, meshgrid, params, N, limit=0.5):
    '''
    input:
        meshgrid: (D, 3)
        wave_fun: (3)-->(N)  a function that calculate the wave function. 
    ...
    return jnp.sum(density ** (4/3)) * const / meshgrid.shape[0]*limit*2


def E_Hartree(wave_fun, meshgrid, params, N,  eps=1e-10, limit=0.5):
    '''
    ...
    density_at_grid = jnp.sum(density_at_grid, axis=0) 
    ...
    density_pair = jax.lax.batch_matmul(jnp.expand_dims(density_at_grid, 1), 
                                        jnp.expand_dims(density_at_grid, 0))
    output = jnp.mean(set_diag_zero(density_pair/(dist_pair+eps)))
    return output*limit*2*limit*2


def E_gs(wave_fun, meshgrid, params, N, nuclei, eps=1e-10, limit=0.5):

    E1 = E_kinetic(wave_fun, meshgrid, params, N, limit)
    E2 = E_ext(wave_fun, meshgrid, nuclei, params, N, eps, limit)
    E3 = E_XC_LDA(wave_fun, meshgrid, params, N, limit)
    E4 = E_Hartree(wave_fun, meshgrid, params, N, eps, limit)
