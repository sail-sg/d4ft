...
def E_kinetic(wave_fun, meshgrid, weight):
  '''
...
def E_ext(wave_fun, meshgrid, nuclei, weight, eps=1e-10):
  '''
    input:
        wave_fun: (3)-->(2, N)
        meshgrid: (D, 3)
  ...
  for k in range(num_ncl):
    ...
    output += jnp.sum(
      wave_at_grid**2 * jnp.expand_dims(x, [1, 2]) *
      jnp.expand_dims(weight, [1, 2])
    )

  return output  # shape: (2, N)


def E_XC_LDA(wave_fun, meshgrid, weight):
  '''
    input:
        meshgrid: (D, 3)
...
def E_Hartree(wave_fun, meshgrid, weight, eps=1e-10):
  '''
  ...
  density_at_grid = vmap(wave_fun)(meshgrid)  # shape: (D,  2,  N)
  density_at_grid = jnp.sum(density_at_grid**2, axis=(1, 2)) * weight
  dist_pair = distmat(meshgrid)

  density_pair = jnp.dot(
    jnp.expand_dims(density_at_grid, 1), jnp.expand_dims(density_at_grid, 0)
  )
...
def E_gs(wave_fun, meshgrid, nuclei, weight, eps=1e-10):

  E1 = E_kinetic(wave_fun, meshgrid, weight)
